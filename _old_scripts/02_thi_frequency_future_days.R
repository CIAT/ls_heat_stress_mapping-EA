...
for (livestock in livestockLS){
  ...
  for (mod in modLS[3]){
    ...
    for (period in periodLS[2]){
      ...
      oDir <- paste0(iDir, "/frequency/days/", mod, "/", period, "/", sep="")
      ...
      severe_stack <- stack(list.files(paste0(oDir), pattern=glob2rx("severe_days*.tif"), full.names = TRUE)) #load thi frequency layers
