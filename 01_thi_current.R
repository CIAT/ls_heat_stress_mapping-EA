...
for (livestock in livestockLS){
  ...
  oDir <- paste0(iDir, "/output/historical/", livestock, "/thi", sep="")
  ...
  for (dy in dateLS){
    ...
    if (!file.exists(paste(oDir, "/thi_", dy, ".tif", sep=""))){
      ...
      cat("Processing THI: ", livestock, dy, "\n")
      
      # load relative humidity layer
      rh <- raster(paste0(iDir, "/data/historical/_tifs/", varLS[1], "/EA__X", dy, ".tif"))
      y<-rh
      
      # read maximum temperature files
      tmax <- raster(paste0(iDir, "/data/historical/_tifs/", varLS[2], "/EA__X", dy, ".tif"))
      ...
      cat(paste("Processed THI: ", livestock, dy, "\n"))
    }
